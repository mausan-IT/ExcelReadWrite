/**
 * SUBMODAL v1.5 Used for displaying DHTML only popups instead of using buggy
 * modal windows.
 * 
...
function initPopUp() {
	...
	popcont.innerHTML = ''
			...
			+ '<img src="images/cancel.png" width="10" height="10" onclick="hidePopWin(false);" id="popCloseBox" />'
			+ '</div>'
			+ '</div>'
			+ '<iframe src="'
			+ gDefaultPage
			+ '" style="width:100%;height:100%;background-color:transparent;" scrolling="auto" frameborder="0" allowtransparency="true" id="popupFrame" name="popupFrame" width="100%" height="100%"></iframe>'
			+ '</div>';
 ...
 * reappear after the dialog is closed. IE has a problem with wanted select form
 * tags to always be the topmost z-index or layer
 */
...
function blanket_size(popUpDivVar, alto) {
...
function window_pos(popUpDivVar, ancho) {
	if (typeof window.innerWidth != 'undefined') {
...
function popup(windowname, alto, ancho) {
	blanket_size(windowname,alto);
	window_pos(windowname, ancho);
	toggle('blanket');
	toggle(windowname);		
}