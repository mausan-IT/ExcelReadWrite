	...
	function inicio_pagina()
		{
			...
			var numCodigo = document.getElementById('numCodigo').value;
			var puedeImprimirse = document.getElementById('puedeImprimirse').value;
			
			
			var secImprimir = document.getElementById('imprimirId');
			if("true" == puedeImprimirse){
				secImprimir.style.display="";
			}else{
				if(numCodigo == 0){
					secImprimir.style.display="none";
				}else{
					secImprimir.style.display="";
				}
			}	
				...
				if("true" != existe_graduacion){	
					...
					if(respuesta == true && respuesta1 == true){
						...
						if("NOGRABAR" != pagina){
							...
							//alert("CMRO guardarConsultaOpt 9");
							...
							var mensaje5 = "\u00BFEst\u00E1 seguro(a) que est\u00E1n correctos?";
							...
							if(siClienteFirmo() && (!fPuedeEditarse())){
								alert("La receta est\u00E1 firmada, no puede ser modificada");
							}else{
								...
								if(respuesta4 == true){						
									...
									var respuesta3 = confirm(mensaje);		
									if(respuesta3){
										document.getElementById('accion').value='modificarConsultaOpt';
										document.forms[0].submit();
									}
								}	
							
							}
						}//Fin de Pagina = NOGRABAR
		...
		function siClienteFirmo(){
			
			var vSiFirmo = document.getElementById('clienteFirmo').checked;
			
			return vSiFirmo;
		}
		
		
		function fPuedeEditarse(){
			
			var resp = true;
			 
			if ("false" == document.getElementById('puedeEditarse').value){
				resp = false;
			}
