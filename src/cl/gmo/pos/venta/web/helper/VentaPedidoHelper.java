...
public class VentaPedidoHelper extends Utils{
    ...
    public void traeDatosFormulario(VentaPedidoForm formulario, HttpSession session)
    {
		...
		//CMRO
		log.warn("CMRO en VentaPedidoHelper, traeDatosFormulario");
		//CMRO
		...
		GraduacionesHelper helper_graduacion = new GraduacionesHelper();
			...
			if(null != prod){
				...
				//if(PosProductoExentoFacade.esProductoExentoLocal(prod.getCod_barra(),formulario.getLocal())) {
					...
					iEsExento = PosProductoExentoFacade.verificarProductoExento(prod.getCod_barra(),formulario.getLocal());
					
					if(0 != iEsExento){
						formulario.setEsExenta(Constantes.STRING_TRUE);
						//Asignando Dcto Exento
						prod.setEsProdExento(Constantes.STRING_TRUE);
						if(1 == iEsExento){
							
							formulario.setEstaAutExenta(Constantes.STRING_TRUE);
							//prod.setDtoMaxExento(90.00);
							prod.setDtoMaxExento(PosProductoExentoFacade.obtenerDctoMaxProdExento(prod.getCod_barra(),formulario.getLocal()));
	...
	public void tarifica_Pedido(VentaPedidoForm formulario){
		...
		//CMRO
		//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido cdg = "+cdg);
		//CMRO
		...
		{
			//CMRO
			//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido, lista de Producto no es vacia ");
			//CMRO
			
			if (lista_productos.size() != Constantes.INT_CERO)
			{				
				//CMRO
				//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido, hay productos");
				//CMRO
				...
				{
					//CMRO
					//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido, total es cero");
					//CMRO
				...
				{
					//CMRO
					//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido, total NO es cero");
					//CMRO
					...
					//CMRO
					/*
					System.out.println("CMRO cant = "+cant);
					System.out.println("CMRO diferencia = "+diferencia);
					System.out.println("CMRO subtotal = "+subtotal);
					System.out.println("CMRO valor = "+valor);
					*/
					//CMRO
				...
				if (formulario.getTotal() == 0) {
					...
					//CMRO
					//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido, true");
					//CMRO
				...
				{
					//CMRO
					//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido, false");
					//CMRO
			...
			{
				//CMRO
				//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido, aqui1");
				//CMRO
		...
		{
			//CMRO
			//System.out.println("CMRO en VentaPedidoHelper -> tarifica_Pedido, aqui2");
			//CMRO
		...
		//CMRO
		System.out.println("CMRO en tarifica, esExenta = "+formulario.getEsExenta());
		System.out.println("CMRO en tarifica, estaAutExenta = "+formulario.getEstaAutExenta());
		//CMRO
	...
	public void ingresaPago(ArrayList<PagoBean> listaPagos,
    	...
    	try {
    		for (PagoBean pago : listaPagos) {
    			...
    			System.out.println("pago.getFpago_vs() ===> "+pago.getFpago_vs());
    			...
    			PosVentaPedidoFacade.insertaPago(formulario.getCodigo_suc() + Constantes.STRING_SLASH + formulario.getCodigo(),
													...
													pago.getNumero_bono(),
													pago.getDescuento(),pago.getRut_vs(),pago.getFpago_vs());
			}
	...
	public String ingresaDocumento(String ticket, int documento, String tipo_doc,
		...
		try {
			...
			System.out.println("CMRO en VentaPedidoHelper, res = "+res);
	...
	public void traePedidosSeleccionado(String codigoPedido, VentaPedidoForm formulario, HttpSession session) {
		...
		int iCantProdExento = 0;
		int verifProdExento = 0;
		int iVerifAutProdExento = 0;
		...
		for (ProductosBean prod : formulario.getListaProductos()) {
			...
			//if(PosProductoExentoFacade.esProductoExentoLocal(prod.getCod_barra(),formulario.getLocal())) {
			verifProdExento = PosProductoExentoFacade.verificarProductoExento(prod.getCod_barra(),formulario.getLocal());
				if(verifProdExento > 0) {
					iCantProdExento++;
					if (verifProdExento == 1){
						iVerifAutProdExento++;
					}
				}
		...
		if(iCantProdExento > 0) {
			formulario.setEsExenta(Constantes.STRING_TRUE);
			//CMRO
				System.out.println("CMRO aqui iCantProdExento");
			//CMRO
			if(iVerifAutProdExento > 0){
				formulario.setEstaAutExenta(Constantes.STRING_TRUE);
				//CMRO
					System.out.println("CMRO aqui1 iVerifAutProdExento");
				//CMRO
			}else{
				//CMRO
					System.out.println("CMRO aqui2 iVerifAutProdExento");
				//CMRO
				formulario.setEstaAutExenta(Constantes.STRING_FALSE);
			}
		}
	...
	/*
	 * LMARIN - 20140714 
	 * Metodo que comprueba si el encargo ya estï¿½ liberado
	 * @ VentaPedidoForm   
	 */
	...
	public String genera_datos_belec(String tipodoc,SeleccionPagoForm formulario,String foliocl,HttpSession session){
		...
		//CMRO
		//System.out.println("CMRO en VentaPedidoHelper, en genera_datos_belec");
		//CMRO
	 ...
	 *@date 20190807
	 */	
	public String genera_datos_gelec(String rutCliente, SeleccionPagoForm formulario,String foliocl,HttpSession session){
		...
		try {					
			ArrayList<ProductosBean> listProductos = (ArrayList<ProductosBean>)session.getAttribute(Constantes.STRING_LISTA_PRODUCTOS);				
			res = util.generaGuiaELEC(rutCliente,folio,formulario,listProductos);
	...
	//CMRO
	public String imprimirTicketCambio(SeleccionPagoForm sform){		
		Utils util = new Utils();
		String resp = "";
		//try {
			resp = util.ticket_cambio2(sform);
			
		/*}
		catch (IOException e) {			
			log.error("IOEXCEPCION  => "+ e.getMessage());
		} catch (Exception e) {
	...
	public void actualizaProductosPorCupon(VentaPedidoForm formulario,String local) {
		...
		//CMRO
		//System.out.println("CMRO VentaPedidoHelper:actualizaProductosPorCupon inicio");
		/*
		if(null == formulario.getListaProductos()) {
			System.out.println("CMRO VentaPedidoHelper:actualizaProductosPorCupon, formulario.getListaProductos() = null");
		}else {
			System.out.println("CMRO VentaPedidoHelper:actualizaProductosPorCupon, formulario.getListaProductos().size = "+formulario.getListaProductos().size());
		}
		*/
		//CMRO
		...
		if (null != formulario.getListaProductos() && formulario.getListaProductos().size() > Constantes.INT_CERO) {
			...
			for (int i = Constantes.INT_CERO; i < formulario.getListaProductos().size(); i++) {
				//CMRO
					//System.out.println("CMRO VentaPedidoHelper:actualizaProductosPorCupon entrando ciclo for");
				...
				//CMRO
				//System.out.println("CMRO prod.getPrevtln() = "+prod.getPrevtln());
				//CMRO
				
				if (Constantes.STRING_BLANCO.equals(prod.getPrevtln())) {
					System.out.println("actualizaProductosPorCupon");
					//System.out.println("CMRO familia = "+prod.getFamilia());
	...
	public String valida_promo_pares(String codigo, String vTienda) throws Exception{
		 ...
		 //CMRO
			System.out.println("CMRO en VentaPedidoHelper, local = "+vTienda);
		 //CMRO
		
		return PosUtilesFacade.valida_promo_pares(codigo, vTienda);
	}
	...
	public void actualizaProductosPromoCadena(VentaPedidoForm formulario,String local) throws Exception {
		System.out.println("PASO actualizaProductosPromoCombo VPH");
		...
		if (null != formulario.getListaProductos() && formulario.getListaProductos().size() > Constantes.INT_CERO) {
			...
			for (int i = Constantes.INT_CERO; i < formulario.getListaProductos().size(); i++) {
				...
				if (Constantes.STRING_BLANCO.equals(prod.getPrevtln())) {
					...
					this.aplicaDescuentoPromoCadena(prod,formulario.getListaProductos(),local);
				}
				formulario.getListaProductos().set(i, prod);
			}
		}
	...
	*/
	...
	/* METODO: Validar si Existe un Producto Exento (codExento) en el arreglo
	 * indiceProd: posicion del elemento en el arreglo
	 * listaProducto: arreglo donde se realizará la revisión
	 * codExento: código de barra del producto a evaluar
	 */
	
	public boolean existeProductoExento(String indiceProd,
			ArrayList<ProductosBean> listaProductos, String vLocal) {
		
	      int iCantIndices = indiceProd.indexOf(",",0);
	      ProductosBean prod = new ProductosBean();
	      boolean bRespuesta = false;
	      
	      if(iCantIndices == -1){
				prod = listaProductos.get(Integer.parseInt(indiceProd));
				//if(codExento.equals(prod.getCod_barra())) {
				if(PosProductoExentoFacade.esProductoExentoLocal(prod.getCod_barra(), vLocal)) {
					bRespuesta = true;
				}
				...
				try{	
					...
					String strArrProd [] = indiceProd.split(",");
					
					for(String iProd : strArrProd){
						prod = listaProductos.get(Integer.parseInt(iProd));
						//if(codExento.equals(prod.getCod_barra())) {
						if(PosProductoExentoFacade.esProductoExentoLocal(prod.getCod_barra(), vLocal)) {
							bRespuesta = true;
							break;
					...
					log.error("Excepcion controlada existeProductoExento ===>"+e.getMessage());
				}
	...
	public boolean existeProductoE(ArrayList<ProductosBean> listaProductos, String vLocal) {
		
	      ProductosBean prod = new ProductosBean();
	      boolean bRespuesta = false;
	      
		if((null != listaProductos) && (0!= listaProductos.size())){
			 
			for (int iCont = 0; iCont < listaProductos.size(); iCont++){
				prod = listaProductos.get(iCont);
				if(PosProductoExentoFacade.esProductoExentoLocal(prod.getCod_barra(),vLocal)){
					bRespuesta = true;
					break;
				}
	...
	public int existeProdExento(ArrayList<ProductosBean> listaProductos, String vLocal) {
		...
		if((null != listaProductos) && (0!= listaProductos.size())){
			...
			for (int iCont = 0; iCont < listaProductos.size(); iCont++){
				prod = listaProductos.get(iCont);
				iResp = PosProductoExentoFacade.verificarProductoExento(prod.getCod_barra(),vLocal);
				if((iResp == 1) || (iResp == 2)) {
					bRespuesta = iResp;
					break;
				}
