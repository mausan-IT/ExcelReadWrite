...
public class SeleccionPagoDispatchActions extends DispatchAction{
			...
			HttpServletResponse response) throws Exception
	{
		...
		//CMRO
		//System.out.println("CMRO en SeleccionPagoDispatch vtipoped = "+vtipoped);
		//CMRO
		...
		/*ASEBASTIAN 20190812*/
		if (Constantes.STRING_VALIDA_GUIA.equals(accion)) 
		{
			//System.out.println("PASO POR SPAGO 2 3 (guia)");
			String resultado[] = new String[6];
			this.carga_formulario(formulario, session, formulario.getFecha());
			//System.out.println("Numero de Guia ==>"+formulario.getNumero_boleta()+"<==>"+local);
			resultado = helper.validaDocumento(formulario, formulario.getNumero_boleta(), local);
		...
		if(Constantes.STRING_PAGAR.equals(accion))
		{
			...
			else
			{
				...
				if (helper.validaCaja(local, formulario.getFecha())) 
				{
					...
					if (Constantes.STRING_PEDIDO.equals(formulario.getOrigen())) 
					{
						...
						//MEJORA 20190514 - PAGO JEAFOSALE
						if (!valida_fpago_OA && pago_seguro.equals("N") && !formulario.getTiene_documentos().equals(Constantes.STRING_TRUE)) {
							System.out.println("STRING_ERROR_FPAGO_OA pasoooooooooooooo 1 "+pago_seguro);
					...
					else
					{
						...
						if (!Constantes.STRING_ALBARAN_DEVOLUCION.equals(formulario.getOrigen())) {
							...
							{
								System.out.println("STRING_ERROR_FPAGO_OA pasoooooooooooooo 2");
								session.setAttribute(Constantes.STRING_ERROR, Constantes.STRING_ERROR_FPAGO_OA);
								formulario.setForma_pago(Constantes.STRING_CERO);
							}
							else
			...
			if (null != formulario.getConvenio() && null != formulario.getConvenio().getId() && !(Constantes.STRING_BLANCO.equals(formulario.getConvenio().getId()))) {
				...
				//NEW CMRO Enero 2020 Caso: Si tiene Doc
				else{
						formulario.setSolo_boleta(Constantes.STRING_TRUE);
						formulario.setSolo_guia(Constantes.STRING_FALSE);
						formulario.setSolo_recaudacion(Constantes.STRING_FALSE);
			...
			if(formulario.getListaPagos().size() > 0){
				...
				try{
					int crb = 0;
					//CMRO
					int contP = 0; //CMRO contador de pagos
					//CMRO
					int pagoGarantia = 0;
					int pagoIsapre = 0;
					int pagoExcedente = 0;
					...
					boolean tieneConvenioIsapre = false;
					boolean tieneConvenioNoIsapre = false;
					boolean tieneConvenioGuiaPorFacturar = false;
					boolean tieneConvenioConBoleta = false;
					
					//CMRO
					String[] arrConv = {"50412","52","88780","27","28","32","34","40","44","1047","1065","134","144","183","176","198","212","213","803","50297","77777","4","56","181","201","792"};
					if((null != formulario.getConvenio()) && (null != formulario.getConvenio().getId())) {
						if((!("null".equals(formulario.getConvenio().getId()))) && (!Constantes.STRING_BLANCO.equals(formulario.getConvenio().getId()))) {
							System.out.println("CMRO campo convenio esta lleno ");
							for (int posicion = 0; posicion < arrConv.length; posicion++) {
								   System.out.println("CMRO formulario.getConvenio().getId() = "+formulario.getConvenio().getId());
								   System.out.println("CMRO arrConv["+posicion+"] = "+arrConv[posicion]);
								   if(formulario.getConvenio().getId().contentEquals(arrConv[posicion])){
									   tieneConvenio = true;
									   break;
								   }
					...
					System.out.println("CMRO tieneConvenio = "+tieneConvenio);
					//System.out.println("CMRO formulario.getConvenio().getId() = "+formulario.getConvenio().getId());
					//System.out.println("CMRO getIsapre() = "+formulario.getConvenio().getIsapre());
					//CMRO
					
					if(tieneConvenio){
						//System.out.println("CMRO en Validando tieneConvenio = "+tieneConvenio);
						tieneConvenioIsapre = ((null!= formulario.getConvenio()) && null!= formulario.getConvenio().getIsapre() && formulario.getConvenio().getIsapre().equals("S"));
						
						tieneConvenioGuiaPorFacturar = formulario.getConvenio().getId().equals(Constantes.STRING_COD_CONVENIO_GUIA_POR_FACTURAR);
						
						//CMRO Validando que sea ConvenioConBoleta
						String[] arrConvBoleta = {"4","56","181","201","792"};
						
						for (int posE = 0; posE < arrConvBoleta.length; posE++) {
							   System.out.println("CMRO formulario.getConvenio().getId() = "+formulario.getConvenio().getId());
							   System.out.println("CMRO arrConvBoleta["+posE+"] = "+arrConvBoleta[posE]);
							   if(formulario.getConvenio().getId().contentEquals(arrConvBoleta[posE])){
								   tieneConvenioConBoleta = true;
								   break;
							   }
						...
						//END CMRO Validando que sea ConvenioConBoleta
					
						if (!tieneConvenioGuiaPorFacturar){
							System.out.println("CMRO aqui1");
							//NEW CMRO Enero 2020
							if(!tieneConvenioConBoleta) {
								System.out.println("CMRO aqui2");
							//END NEW Enero 2020
								tieneConvenioNoIsapre = ((null!= formulario.getConvenio()) && (null== formulario.getConvenio().getIsapre() || (!formulario.getConvenio().getIsapre().equals("S"))));
							}	
						}
						
						System.out.println("CMRO tieneConvenio, tieneConvenioIsapre = "+tieneConvenioIsapre);
						System.out.println("CMRO tieneConvenio, tieneConvenioNoIsapre = "+tieneConvenioNoIsapre);
						System.out.println("CMRO tieneConvenio, tieneConvenioGuiaPorFacturar = "+tieneConvenioGuiaPorFacturar);
						System.out.println("CMRO tieneConvenio, tieneConvenioConBoleta = "+tieneConvenioConBoleta);
					}
					
					System.out.println("CMRO tieneConvenioIsapre = "+tieneConvenioIsapre);
					System.out.println("CMRO tieneConvenioNoIsapre = "+tieneConvenioNoIsapre);
					System.out.println("CMRO tieneConvenioGuiaPorFacturar = "+tieneConvenioGuiaPorFacturar);
					System.out.println("CMRO tieneConvenioConBoleta = "+tieneConvenioConBoleta);
					
					
					
					//CMRO
					for(PagoBean b : formulario.getListaPagos()){
						if((b.getForma_pago().equals("ISAPR") || b.getForma_pago().equals("CRB") ) && b.isTiene_doc() == true){
							//CMRO
							System.out.println("CMRO b.getForma_pago().equals(ISAPR) && b.isTiene_doc() == true");
							//CMRO
						...
						//CMRO
							...
							System.out.println("CMRO b.getForma_pago() = "+b.getForma_pago()+" , b.isTiene_doc() = "+b.isTiene_doc());
							
							//CMRO
							if (helper.pagoConEstaFormaDePago(b.getForma_pago(),"GAR", b.isTiene_doc())) {pagoGarantia = 1;}
							if (helper.pagoConEstaFormaDePago(b.getForma_pago(),"EXCED", b.isTiene_doc())) {pagoExcedente = 1;}
							if (helper.pagoConEstaFormaDePago(b.getForma_pago(),"ISAPR", b.isTiene_doc()) || helper.pagoConEstaFormaDePago(b.getForma_pago(),"CRB", b.isTiene_doc()))
							{
								pagoIsapre = 1;
							}
							if (helper.pagoConEstaFormaDePago(b.getForma_pago(),"OA", b.isTiene_doc()) || helper.pagoConEstaFormaDePago(b.getForma_pago(),"OASD", b.isTiene_doc()))
							{
								pagoOA = 1;
							}
						}
						...
						System.out.println("CMRO crb = "+crb);
						System.out.println("CMRO contP = "+contP);
						System.out.println("CMRO pagoExcedente = "+pagoExcedente);
						System.out.println("CMRO pagoISAPRE = "+pagoIsapre);
						System.out.println("CMRO pagoGarantia = "+pagoGarantia);
						System.out.println("CMRO pagoOA = "+pagoOA);
					...
					System.out.println("CMRO fuera del for crb = "+crb);
					System.out.println("CMRO fuera del for contP = "+contP);
					System.out.println("CMRO pagoExcedente = "+pagoExcedente);
					System.out.println("CMRO pagoISAPRE = "+pagoIsapre);
					System.out.println("CMRO pagoGarantia = "+pagoGarantia);
					System.out.println("CMRO pagoOA = "+pagoOA);
					...
					/*if(null!= formulario.getConvenio() && null!= formulario.getConvenio().getIsapre() && formulario.getConvenio().getIsapre().equals("S") &&  crb == 1){
						System.out.println("paso solo boleta");
						formulario.setSolo_boleta(Constantes.STRING_TRUE);
					}else{
						
						//CMRO
						System.out.println("CMRO NO formulario.getConvenio().getIsapre().equals(S)  OR no crb === 1, crb = "+crb);
						if (pagoGarantia && tieneConvenioIsapre && (!pagoIsapre) && (!pagoExcedente))
						{
							formulario.setSolo_guia(Constantes.STRING_TRUE);
							formulario.setSolo_recaudacion(Constantes.STRING_FALSE);
							formulario.setSolo_boleta(Constantes.STRING_FALSE);
							
							System.out.println("tiene gar y tiene mas de un pago");
						}
						else
						{}
						//CMRO
					...
					if (tieneConvenio) {
						if (pagoGarantia > 0) {
							...
							System.out.println("CMRO en pagoGarantia true");
							System.out.println("CMRO en tieneConvenioIsapre = "+tieneConvenioIsapre);
							System.out.println("CMRO en tieneConvenioGuiaPorFacturar = "+tieneConvenioGuiaPorFacturar);
						...
						if(tieneConvenioGuiaPorFacturar){	
							...
							if(formulario.getDiferencia() > 0) {
								...
								System.out.println("CMRO en tieneConvenioGuiaPorFacturar");
							    ...
							    if(tieneConvenioConBoleta) {
									...
									//CMRO
									System.out.println("CMRO en tieneConvenioConBoleta");
									//CMRO
									...
									if(tieneConvenioIsapre && (0 == pagoIsapre) && (0 == pagoExcedente)) {
										
										
										System.out.println("CMRO en tieneConvenioIsapre && (!pagoIsapre) && (!pagoExcedente)");
										...
										if(tieneConvenioIsapre && ((1 == pagoIsapre) || (1 == pagoExcedente))){
										
											
											System.out.println("CMRO no ha pagado Convenio");
											...
											if (tieneConvenioNoIsapre && (0 == pagoOA)) {
												formulario.setSolo_guia(Constantes.STRING_TRUE);
												formulario.setSolo_boleta(Constantes.STRING_FALSE);
												formulario.setSolo_recaudacion(Constantes.STRING_FALSE);
											}
									...
									}//END IF tieneConvenioIsapre
								}//END ELSE tieneConvenioConBoleta
						}//END ELSE tieneConvenioGuiaPorFacturar
								...
								formulario.setSolo_recaudacion(Constantes.STRING_FALSE);
									...
									if(tieneConvenioGuiaPorFacturar) {
										...
										if(formulario.getDiferencia() > 0) {
											...
											//System.out.println("CMRO tieneConvenioGuiaPorFacturar");
											...
											if(tieneConvenioConBoleta) {
												...
												//CMRO
												System.out.println("CMRO en tieneConvenioConBoleta aqui");
												//CMRO
												...
												if(formulario.getDiferencia() > 0) {
													...
													//CMRO
												System.out.println("CMRO en tieneConvenioConBoleta aqui, formulario.getDiferencia() ="+formulario.getDiferencia());
												//CMRO
													...
													System.out.println("CMRO No tieneConvenioConBoleta");
													if (tieneConvenioNoIsapre && (0 == pagoOA)) {
														formulario.setSolo_guia(Constantes.STRING_TRUE);
														formulario.setSolo_boleta(Constantes.STRING_FALSE);
														formulario.setSolo_recaudacion(Constantes.STRING_FALSE);
					...
					//GAR
					//OJO CON ESTA VALIDACION VALIDAR QUE VAYA ESN ESTA SECCION
					formulario.setTiene_fomas_pago("SI");
		...
		if(Constantes.STRING_IMPRIME_GUIA.equals(accion))
		{
		  ...
		  if(pago_seguro.equals("S") || vtipoped.equals("S") ){
			    if(null == formulario.getConvenio().getId() || Constantes.STRING_BLANCO.equals(formulario.getConvenio().getId())) {
			    	System.out.println("PASOOOOOOOOOOOOO CONVENIO SEGUROOOOOOOOOO (1)");
			    	...
			    	System.out.println("\"PASOOOOOOOOOOOOO CONVENIO SEGUROOOOOOOOOO (2)"+!formulario.getConvenio().getIsapre().equals("S"));
			    	session.setAttribute(Constantes.STRING_CABECERA_GUIA, formulario);
					session.setAttribute(Constantes.STRING_ERROR, Constantes.STRING_IMPRIME_GUIA);
					//CMRO formulario.setTipo_doc(Constantes.CHAR_G);
					formulario.setTipo_doc(Constantes.CHAR_R);
		  		...
		  		if (null == formulario.getConvenio().getId() || "null".equals(formulario.getConvenio().getId()) || Constantes.STRING_BLANCO.equals(formulario.getConvenio().getId())) {
					...
					if (formulario.getDiferencia() > 0  && !formulario.getConvenio().getIsapre().equals("S") || pago_seguro.equals("S") || vtipoped.equals("S")) {
				    	System.out.println("PASOOOOOOOOOOOOO CONVENIO SEGUROOOOOOOOOO (3");
						System.out.println("paso impresion docuemnto vta seguro 1");
						session.setAttribute(Constantes.STRING_ERROR, Constantes.STRING_VALIDACION_TOTAL_PAGADO);				
					}
					else
					{
				    	System.out.println("PASOOOOOOOOOOOOO CONVENIO SEGUROOOOOOOOOO (4)");
						session.setAttribute(Constantes.STRING_CABECERA_GUIA, formulario);
						session.setAttribute(Constantes.STRING_ERROR, Constantes.STRING_IMPRIME_GUIA);
				...
				else
				{
					...
					{
				    	System.out.println("PASOOOOOOOOOOOOO CONVENIO SEGUROOOOOOOOOO (5)");
						session.setAttribute(Constantes.STRING_CABECERA_GUIA, formulario);
						session.setAttribute(Constantes.STRING_ERROR, Constantes.STRING_IMPRIME_GUIA);
						formulario.setTipo_doc(Constantes.CHAR_G);
					}
					else
					{
						//20140630  cruz blanca 50412 || se agrega venta seguro
						if (formulario.getDiferencia() > 0 && !formulario.getConvenio().getIsapre().equals("S") || pago_seguro.equals("S") || vtipoped.equals("S")) {
					    	System.out.println("PASOOOOOOOOOOOOO CONVENIO SEGUROOOOOOOOOO (6)");
							System.out.println("paso impresion docuemnto vta seguro 2");
							session.setAttribute(Constantes.STRING_ERROR, Constantes.STRING_VALIDACION_TOTAL_PAGADO);
						}
						else
						{
					    	System.out.println("PASOOOOOOOOOOOOO CONVENIO SEGUROOOOOOOOOO (7)");
							session.setAttribute(Constantes.STRING_CABECERA_GUIA, formulario);
							session.setAttribute(Constantes.STRING_ERROR, Constantes.STRING_IMPRIME_GUIA);
							formulario.setTipo_doc(Constantes.CHAR_G);
						}
	 ...
	 *@FECHA 20181025
     */
	 public ActionForward exige_validacion_fpago(ActionMapping mapping,
	            ActionForm form,
	            HttpServletRequest request,
	            HttpServletResponse response) throws Exception
     {
		  ...
		  SeleccionPagoForm formulario = (SeleccionPagoForm)form;	
		  int estado = 0;
		  estado = helper.exige_validacion_fpago(formulario.getMotivo(),formulario.getFpago());
		  System.out.println("exige_validacion_fpago personal==> "+estado);
		  response.getWriter().print(estado);		
		  return null;
      }
	 
	 public ActionForward abre_valida_encargo_mf(ActionMapping mapping,
	            ActionForm form,
	            HttpServletRequest request,
	            HttpServletResponse response) throws Exception
	 {
		 return mapping.findForward(Constantes.STRING_ACTION_VALIDA_ENCARGO_MF);
	 }
	
	 public ActionForward validacion_encargo_fpago (ActionMapping mapping,
	            ActionForm form,
	            HttpServletRequest request,
	            HttpServletResponse response) throws Exception
	  {
		  SeleccionPagoForm formulario = (SeleccionPagoForm)form;	
		  int estado = 0;
		  estado = helper.validacion_encargo_fpago(formulario.getEncargo_rel(),formulario.getSerie(),formulario.getMotivo());
		  System.out.println("validacion_encargo_fpago ==> "+formulario.getEncargo_rel()+" "+formulario.getSerie()+" "+formulario.getMotivo());
		  response.getWriter().print(estado);		
		  return null;
	  }
