		...
		if (Constantes.STRING_PAGO_EXITOSO.equals(formulario.getAccion()))
		{
			...
			if (!formulario.getEstado().equals("ERROR_GUARDADO")) {
			    ...
			    if (session.getAttribute(Constantes.STRING_TIPO_DOCUMENTO).toString().trim().equals("G")) {
					...
					//CMRO Considerando impresión de Boleta Exenta
					String vTipoDocImp = "BOLETA-1";
					if (Constantes.STRING_TRUE.contentEquals(formulario.getEsExenta())) {
						vTipoDocImp = "BOLETA-2";
					}
						
			    	out2 = ventaHelper.genera_datos_belec(vTipoDocImp, spagoform, folio[1], session);
			    }
